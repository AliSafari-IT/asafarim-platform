...
> **Platform port notes.** This document was written against the original
> `asafarim-digital` implementation and describes its behavior; the app has
> since been ported into this monorepo (issue #26). Differences in the port:
> the dev server runs on port **3004** (not 3006); centralized sign-in lives
> on the **Hub** app (`hub.asafarim.com`), replacing every "portal" reference;
> the DB-driven `/api/navigation` registry and its `@asafarim/navigation` /
> `@asafarim/types` dependencies were replaced by a static in-app nav
> (`components/ViontoNav.tsx`) and theme helpers local to the app
> (`lib/theme.ts`); the platform has no multi-tenancy, so `tenantId` was
> dropped from `AuthedUser` and the Vionto Prisma models (kept only as a plain
> optional column on `ViontoUsageMetric`); production runs behind **Caddy**
> (`infra/caddy/Caddyfile`) instead of Nginx, with compose services `vionto`,
> `vionto-worker`, and `redis` in `docker-compose.prod.yml`. Everything else
> below matches the ported code.
...
| Auth configuration | `packages/auth/src/index.ts`, `packages/auth/src/config.ts`, `packages/auth/src/proxy.ts` |
...
| Story/vision/SRT | `apps/vionto/app/api/story/**`, `apps/vionto/lib/server/story-generation.ts`, `vision.ts`, `srt.ts`, `exif.ts` |
...
| Export metadata/download | `apps/vionto/lib/server/export-metadata.ts`, `apps/vionto/app/api/exports/**` |
| Google Photos | `apps/vionto/lib/server/google-photos/**`, `apps/vionto/app/api/integrations/google-photos/**` |
| Production wiring | `apps/vionto/Dockerfile`, `infra/docker/Dockerfile.vionto-worker`, `docker-compose.prod.yml`, `infra/caddy/Caddyfile` |
