...
Status: design only. This document proposes a future demo seed; it does not
change the current seed implementation or application code.
...
notification, and admin states. The current seed has 3 students, 3 tutors, and
one booking chain. This proposal expands it to 50 members and deliberately
covers the features added through the 2026-08-12 repository state, including
...
| S03 | Elias Bernard | 17, K12 | fr, online | Independent minor; five candidates; mixed proposal states and newcomer slot |
...
| Learning Briefs | 18 foreground | 4 DRAFT, 4 CONFIRMED, 6 MATCHED, 4 ARCHIVED; all three triage outcomes |
| Intake turns | 70-90 | MESSAGE, QUESTION, HELP, SUMMARY; no repeated target field within a brief |
| Match candidates | 25-30 | 1-5 per applicable brief; persisted eight-factor breakdown; at least 2 newcomer rotations |
| Recent/foreground inquiries | 18 | All legacy statuses: NEW, AI_RESPONDED, TUTOR_REQUESTED, BOOKED, CLOSED, REFUSED |
| Historical inquiry/quote chains | About 55 | Mostly CLOSED/BOOKED; terse, clearly labelled synthetic history |
| Quote requests | About 70 | Foreground OPEN/FULFILLED/EXPIRED/CANCELLED plus historical fulfilled requests |
| Quotes/proposals | About 90 | DRAFT, PENDING, ACCEPTED, DECLINED, EXPIRED; only PENDING/accepted proposals have `sentAt` |
| Bookings | About 70 | 6 SCHEDULED, 56 COMPLETED, 4 CANCELLED, 4 DISPUTED |
| Session records | About 60 | 56 ATTENDED, 2 PARTIAL, 2 NO_SHOW; records align with booking lifecycle |
| Reviews | About 51 | One per reviewed COMPLETED booking; aspect ratings on recent reviews; a few legacy overall-only reviews |
| Transactions | 75-90 | CHARGE, PLATFORM_FEE, REFUND, PAYOUT; amounts reconcile per booking/tutor |
| Wallets | 11 | Available, pending, below threshold, eligible, paid recently, and zero-balance examples |
| Notifications | 80-120 | Every supported event type; read/unread and preference-suppressed cases |
| Verification records | 15-20 | Current state plus history for re-review/needs-changes cases |
| Verification messages | 8-12 | Both sender roles, read/unread, attachment, and supported reactions |
| Audit events | 120+ | Creation and every sensitive state transition represented |
...
## 12. Acceptance checklist for a future implementation conversation

- [ ] Exactly 50 synthetic users are present and removable.
- [ ] Exactly 15 tutor profiles exist: 10 hybrid and 5 online-only.
...
- [ ] Match previews never exceed five candidates and contain the eight-factor
      explanation.
- [ ] At least one minor filter, proximity filter, unverified filter, and
      newcomer rotation are visible in admin matching diagnostics.
- [ ] Rating aggregates and tutor resumes are recomputed from the seeded
      session/review history.
- [ ] Future/scheduled dates, dispute windows, streaks, and payout holding
      periods remain correct relative to `T0`.
- [ ] The application makes no external provider calls while seeding or merely
      browsing the demo.
