...
import { getEnvStatus, loadConfig } from "@asafarim/envage";
...
function isSensitivePlaintext(file) {
...
const trackedSensitive = gitFiles(["ls-files"]).filter(isSensitivePlaintext);
const stagedSensitive = gitFiles(["diff", "--cached", "--name-only"]).filter(
