...
async function currentActiveReleaseId(
  db: Db,
  appId: string
): Promise<string | null> {
...
export async function createDeployment(
  ...
  const { app } = await assertCapability(db, actor, appId, "app.deployRelease");
  const requestHash = checksumOf({ appId, releaseId: input.releaseId });

  return withQuotaRejectionLogging(
    db,
    { category: "deployment", appId, actorPrincipalId: actor.principalId },
    () =>
      db.transaction(async (tx) => {
        ...
        if (existing) {
          if (existing.requestHash !== requestHash) {
            throw new ConflictError(
              "Idempotency key reused with a different deployment request payload."
            );
          }
          return existing;
        }
        ...
        if (release.status !== "approved") {
          throw new ConflictError(
            `Release is in status "${release.status}" — only an APPROVED release can be deployed.`
          );
        }

        const eligibility = await checkReleaseEligibility(
          tx,
          appId,
          release.specificationVersionId
        );
        ...
        // M12: a forward deploy is gated by a race-safe concurrency quota — no
        // such check existed pre-M12 (a bare SELECT-then-INSERT would have the
        // same double-insert race documented on lib/quotas/enforce.ts#withQuota).
        // Rollback (below) deliberately bypasses this — an emergency rollback
        // must never be blocked by a concurrency quota meant to bound normal
        // forward-deploy churn.
        return withQuota(
          tx,
          appId,
          "concurrent_deployment_jobs_per_app",
          () => countConcurrentDeploymentJobsForApp(tx, appId),
          async () => {
...
export async function rollbackToRelease(
  ...
  return db.transaction(async (tx) => {
    ...
    if (existing) {
      if (existing.requestHash !== requestHash) {
        throw new ConflictError(
          "Idempotency key reused with a different rollback request payload."
        );
      }
      return existing;
    }
    ...
    const [target] = await tx
      .select()
      .from(releases)
      .where(eq(releases.id, input.targetReleaseId))
      .limit(1);
    ...
    if (!target.publishedAt) {
      throw new RollbackTargetInvalidError(
        "This release was never published to production and cannot be rolled back to."
      );
    }
    ...
    if (supersededReleaseId === target.id) {
      throw new ConflictError(
        "This release is already the active production release."
      );
    }
...
export async function claimDeploymentById(
  db: Db,
  deploymentId: string,
  workerId: string,
...
export async function claimNextAvailableDeployment(
  db: Db,
  workerId: string,
  leaseDurationMs: number
): Promise<DeploymentRow | null> {
...
async function claimInternal(
  ...
  return db.transaction(async (tx) => {
    ...
    const baseCondition = and(
      ...
      or(
        isNull(deployments.leaseExpiresAt),
...
export async function heartbeat(
  ...
  const [updated] = await db
    ...
    .where(
      and(
        eq(deployments.id, deploymentId),
        eq(deployments.leaseOwner, workerId),
        notInArray(deployments.status, TERMINAL_STATUS_LIST)
      )
    )
...
export async function transitionPhase(
  ...
  const [updated] = await db
    ...
    .where(
      and(eq(deployments.id, deploymentId), eq(deployments.phase, fromPhase))
    )
    .returning();
...
export async function requestDeploymentCancellation(
  ...
  return db.transaction(async (tx) => {
    ...
    if (isTerminalStatus(deployment.status)) {
      throw new ConflictError(
        `Deployment already finished with status "${deployment.status}"; it cannot be cancelled.`
      );
    }
    if (deployment.activatedAt) {
      throw new ConflictError(
        "This deployment already activated the production pointer — cancel is no longer possible; roll back instead."
      );
    }
    ...
    await recordAuditEvent(tx, {
      appId,
      actorPrincipalId: actor.principalId,
      action: "deployment.cancellation_requested",
      targetType: "deployment",
      targetId: deploymentId,
      metadata: {},
    });
...
export function isCancellationRequested(deployment: DeploymentRow): boolean {
  return (
    deployment.cancelRequestedAt !== null || deployment.status === "cancelled"
  );
}
...
export async function recordDeploymentStep(
  db: Db,
  input: RecordDeploymentStepInput
): Promise<DeploymentStepRow> {
  const safeMessage = String(
    (redactDiagnostics({ message: input.message }) as { message?: unknown })
      .message ?? ""
  );
...
export async function listDeploymentSteps(
  ...
  return db
    ...
    .orderBy(asc(deploymentSteps.createdAt));
}

// ─── Reads ──────────────────────────────────────────────────────────────

export async function getDeploymentForActor(
  db: Db,
  actor: Actor,
  appId: string,
  deploymentId: string
...
export async function listDeploymentsForActor(
  db: Db,
  actor: Actor,
  appId: string,
  limit = 20
...
export async function getDeploymentStepsForActor(
  db: Db,
  actor: Actor,
  appId: string,
  deploymentId: string
