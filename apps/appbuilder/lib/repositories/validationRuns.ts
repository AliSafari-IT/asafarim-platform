...
import { assertTransition, isTerminal, TERMINAL_STATUSES, type ValidationRunStatus } from "../validation/stateMachine";
import { VALIDATION_LIMITS } from "../validation/limits";
import { GATE_SET_VERSION, GATE_DEFINITIONS } from "../validation/gates/registry";
import { computeReleaseEligibility, countMandatoryPassed } from "../validation/eligibility";
...
export class ValidationRunLeaseLostError extends ConflictError {
  constructor(runId: string) {
    super(`Lease on validation run ${runId} was lost (reclaimed by another worker or run no longer active)`);
...
export class StaleValidationRunStateError extends ConflictError {
  constructor(runId: string, expected: string) {
    super(`Validation run ${runId} was not in the expected status "${expected}" (concurrent update)`);
...
export async function enqueueValidationRun(
  ...
  await assertCapability(db, actor, appId, "app.validate");
  const requestHash = checksumOf({ appId, requestSource: input.requestSource, triggeringRepairAttemptId: input.triggeringRepairAttemptId ?? null });

  return db.transaction(async (tx) => {
    const [existing] = await tx
      .select()
      .from(validationRuns)
      .where(and(eq(validationRuns.appId, appId), eq(validationRuns.idempotencyKey, input.idempotencyKey)))
      .limit(1);
    if (existing) {
      if (existing.requestHash !== requestHash) {
        throw new ConflictError("Idempotency key reused with a different validation request payload");
      }
      return existing;
    }
    ...
    const activeForApp = await tx
      ...
      .from(validationRuns)
      .where(and(eq(validationRuns.appId, appId), notInArray(validationRuns.status, TERMINAL_STATUS_LIST)));
    if (activeForApp.length >= VALIDATION_LIMITS.MAX_ACTIVE_RUNS_PER_APP) {
      throw new ConflictError(
        `This app already has ${activeForApp.length} active validation run(s) (limit ${VALIDATION_LIMITS.MAX_ACTIVE_RUNS_PER_APP}). Wait for it to finish, or cancel it first.`,
      );
    }
    ...
    const [version] = await tx
      ...
      .limit(1);
    if (!version) throw new NotFoundError("Specification version", `${spec.id}@${spec.currentVersionNumber}`);
    ...
    const [run] = await tx
      ...
      .values({
        ...
        previewBuildId: existingBuild && existingBuild.status === "succeeded" ? existingBuild.id : null,
        ...
        requestHash,
        mandatoryGatesTotal: GATE_DEFINITIONS.filter((g) => g.mandatory).length,
      })
      .returning();
    ...
    await recordAuditEvent(tx, {
      ...
      targetId: run.id,
      metadata: { requestSource: input.requestSource, specificationVersionId: version.id },
    });
...
export async function claimRunById(db: Db, runId: string, workerId: string, leaseDurationMs: number): Promise<ValidationRunRow | null> {
  return claimInternal(db, workerId, leaseDurationMs, eq(validationRuns.id, runId));
}

export async function claimNextAvailableRun(db: Db, workerId: string, leaseDurationMs: number): Promise<ValidationRunRow | null> {
  return claimInternal(db, workerId, leaseDurationMs, undefined);
}

async function claimInternal(db: Db, workerId: string, leaseDurationMs: number, extraCondition: SQL | undefined): Promise<ValidationRunRow | null> {
  return db.transaction(async (tx) => {
    const now = new Date();
    const baseCondition = and(
      notInArray(validationRuns.status, TERMINAL_STATUS_LIST),
      or(isNull(validationRuns.leaseExpiresAt), lt(validationRuns.leaseExpiresAt, now)),
    );
...
export async function heartbeat(db: Db, runId: string, workerId: string, leaseDurationMs: number): Promise<void> {
  const now = new Date();
  const [updated] = await db
    .update(validationRuns)
    .set({ heartbeatAt: now, leaseExpiresAt: new Date(now.getTime() + leaseDurationMs), updatedAt: now })
    .where(and(eq(validationRuns.id, runId), eq(validationRuns.leaseOwner, workerId), notInArray(validationRuns.status, TERMINAL_STATUS_LIST)))
    .returning({ id: validationRuns.id });
  if (!updated) throw new ValidationRunLeaseLostError(runId);
}

export async function releaseLease(db: Db, runId: string, workerId: string): Promise<void> {
  await db
    .update(validationRuns)
    .set({ leaseOwner: null, leaseExpiresAt: null, updatedAt: new Date() })
    .where(and(eq(validationRuns.id, runId), eq(validationRuns.leaseOwner, workerId)));
}
...
export async function requestCancellation(db: Db, actor: Actor, appId: string, runId: string): Promise<ValidationRunRow> {
  ...
  return db.transaction(async (tx) => {
    ...
    if (isTerminal(run.status)) {
      throw new ConflictError(`Validation run already finished with status "${run.status}"; it cannot be cancelled.`);
    }
    ...
    if (run.status === "pending") {
      const [updated] = await tx
        ...
        .returning();
      await recordAuditEvent(tx, { appId, actorPrincipalId: actor.principalId, action: "validation.cancelled", targetType: "validation_run", targetId: runId, metadata: {} });
      return updated;
    }
    ...
    const [updated] = await tx
      ...
      .where(eq(validationRuns.id, runId))
      .returning();
    await recordAuditEvent(tx, { appId, actorPrincipalId: actor.principalId, action: "validation.cancellation_requested", targetType: "validation_run", targetId: runId, metadata: {} });
    return updated;
  });
}
...
export async function upsertGateResult(
  ...
  const [row] = await db
    ...
    .values({
      ...
      structuredFailures: (input.result.structuredFailures as unknown as Record<string, unknown>[] | undefined) ?? [],
...
export async function listGateResultsForRun(db: Db, runId: string): Promise<ValidationGateResultRow[]> {
  return db.select().from(validationGateResults).where(eq(validationGateResults.runId, runId)).orderBy(asc(validationGateResults.createdAt));
}
...
export async function finalizeRun(
  db: Db,
  runId: string,
  finalStatus: Extract<ValidationRunStatus, "passed" | "failed" | "infrastructure_error" | "flaky">,
): Promise<ValidationRunRow> {
  const gates = await listGateResultsForRun(db, runId);
  const { total, passed } = countMandatoryPassed(gates);
  const releaseEligible = finalStatus === "passed" && computeReleaseEligibility(gates);
...
export async function getValidationRunForActor(db: Db, actor: Actor, appId: string, runId: string): Promise<ValidationRunRow> {
  await assertCapability(db, actor, appId, "app.viewValidation");
  const [run] = await db.select().from(validationRuns).where(and(eq(validationRuns.id, runId), eq(validationRuns.appId, appId))).limit(1);
  if (!run) throw new NotFoundError("Validation run", runId);
  return run;
}

export async function listValidationRunsForActor(db: Db, actor: Actor, appId: string, limit = 20): Promise<ValidationRunRow[]> {
  await assertCapability(db, actor, appId, "app.viewValidation");
  return db.select().from(validationRuns).where(eq(validationRuns.appId, appId)).orderBy(desc(validationRuns.createdAt)).limit(limit);
}

export async function getLatestValidationRunForActor(db: Db, actor: Actor, appId: string): Promise<ValidationRunRow | null> {
  await assertCapability(db, actor, appId, "app.viewValidation");
  const [row] = await db.select().from(validationRuns).where(eq(validationRuns.appId, appId)).orderBy(desc(validationRuns.createdAt)).limit(1);
  return row ?? null;
}

export async function getGateResultsForActor(db: Db, actor: Actor, appId: string, runId: string): Promise<ValidationGateResultRow[]> {
...
export async function getArtifactsForActor(db: Db, actor: Actor, appId: string, runId: string): Promise<ValidationArtifactRow[]> {
  await getValidationRunForActor(db, actor, appId, runId);
  return db.select().from(validationArtifacts).where(and(eq(validationArtifacts.runId, runId), eq(validationArtifacts.appId, appId)));
}
