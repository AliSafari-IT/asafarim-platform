...
describe("applyOperation — transactional persistence", () => {
  it("creates an immutable version, an append-only operation record, and an audit event atomically", async () => {
    ...
    expect((versions[0].payload as any).entities.some((e: any) => e.id === "widget")).toBe(true);
  });
});
...
describe("optimistic concurrency", () => {
  it("rejects a stale base version, preserving the winning edit and never applying the losing one", async () => {
    const app = await makeApp();
    await applyOperation(db, owner, app.id, { operation: createEntityOp("first"), baseVersionNumber: 0, idempotencyKey: "k-a" });

    await expect(
      applyOperation(db, owner, app.id, { operation: createEntityOp("second"), baseVersionNumber: 0, idempotencyKey: "k-b" }),
    ).rejects.toBeInstanceOf(StaleVersionError);

    const versions = await listVersionsForActor(db, owner, app.id);
    expect(versions).toHaveLength(1); // the stale write never landed
    expect((versions[0].payload as any).entities.map((e: any) => e.id)).toEqual(["first"]);
  });
  ...
  it("under real concurrent requests, exactly one of two racing writers wins", async () => {
    ...
    const results = await Promise.allSettled([
      applyOperation(db, owner, app.id, { operation: createEntityOp("race_a"), baseVersionNumber: 0, idempotencyKey: "race-a" }),
      applyOperation(db, owner, app.id, { operation: createEntityOp("race_b"), baseVersionNumber: 0, idempotencyKey: "race-b" }),
    ]);
    ...
    const versions = await listVersionsForActor(db, owner, app.id);
    expect(versions).toHaveLength(1); // no duplicate version from the race
  });
});
...
describe("idempotency", () => {
  it("replays the same result for a repeated key + identical payload — no duplicate rows", async () => {
    const app = await makeApp();
    const input = { operation: createEntityOp("widget"), baseVersionNumber: 0, idempotencyKey: "same-key" };
  ...
  it("rejects the same key reused with a different payload, without creating a second version", async () => {
    const app = await makeApp();
    await applyOperation(db, owner, app.id, { operation: createEntityOp("widget"), baseVersionNumber: 0, idempotencyKey: "conflict-key" });

    await expect(
      applyOperation(db, owner, app.id, { operation: createEntityOp("other"), baseVersionNumber: 0, idempotencyKey: "conflict-key" }),
...
describe("transaction rollback on validation failure", () => {
  ...
  it("rejects a structurally invalid operation payload (OperationValidationError) with no persisted state", async () => {
    const app = await makeApp();
    await applyOperation(db, owner, app.id, { operation: createEntityOp("existing"), baseVersionNumber: 0, idempotencyKey: "k1" });
    ...
    const versionRows = await db.select().from(specificationVersions).where(eq(specificationVersions.appId, app.id));
    expect(versionRows).toHaveLength(1); // only the first, successful one
  });
});
...
describe("destructive confirmation", () => {
  it("rejects a destructive operation without confirmDestructive and persists nothing", async () => {
    const app = await makeApp();
    await applyOperation(db, owner, app.id, { operation: createEntityOp("widget"), baseVersionNumber: 0, idempotencyKey: "k1" });
...
describe("restore as a new version", () => {
  it("restores an earlier version's payload as a brand-new version, without touching history", async () => {
    const app = await makeApp();
    await applyOperation(db, owner, app.id, { operation: createEntityOp("v1_entity"), baseVersionNumber: 0, idempotencyKey: "k1" });
    await applyOperation(db, owner, app.id, { operation: createEntityOp("v2_entity"), baseVersionNumber: 1, idempotencyKey: "k2" });

    const v1Before = await getVersionForActor(db, owner, app.id, 1);

    const { version: v3 } = await restoreVersion(db, owner, app.id, {
    ...
    expect((v3.payload as any).entities.map((e: any) => e.id)).toEqual(["v1_entity"]);

    // History is untouched.
    const v1After = await getVersionForActor(db, owner, app.id, 1);
    expect(v1After.checksum).toBe(v1Before.checksum);
    const v2 = await getVersionForActor(db, owner, app.id, 2);
    expect((v2.payload as any).entities.map((e: any) => e.id)).toEqual(["v1_entity", "v2_entity"]);
  });
  ...
  it("rejects a stale restore request", async () => {
    ...
    await expect(
      restoreVersion(db, owner, app.id, { targetVersionNumber: 1, baseVersionNumber: 0, idempotencyKey: "restore-stale" }),
    ).rejects.toBeInstanceOf(StaleVersionError);
  });
});
...
describe("safe undo via inverse operation", () => {
  it("undoes a CREATE_ENTITY by archiving the entity it created", async () => {
    const app = await makeApp();
    await applyOperation(db, owner, app.id, { operation: createEntityOp("undoable"), baseVersionNumber: 0, idempotencyKey: "k1" });

    const { version } = await undoLastOperation(db, owner, app.id, { baseVersionNumber: 1, idempotencyKey: "undo-1" });
  ...
  it("returns RestoreRequiredError when the last operation has no safe inverse", async () => {
    const app = await makeApp();
    await applyOperation(db, owner, app.id, { operation: createEntityOp("permanent"), baseVersionNumber: 0, idempotencyKey: "k1" });
    ...
    await expect(
      undoLastOperation(db, owner, app.id, { baseVersionNumber: 2, idempotencyKey: "undo-2" }),
...
describe("compare (diff)", () => {
  it("reports the entity added between two versions", async () => {
    ...
    const diff = await compareVersionsForActor(db, owner, app.id, 1, 2);
...
describe("released-version immutability", () => {
  it("never changes an earlier version's checksum as later operations are applied", async () => {
    const app = await makeApp();
    await applyOperation(db, owner, app.id, { operation: createEntityOp("v1_entity"), baseVersionNumber: 0, idempotencyKey: "k1" });
    const v1 = await getVersionForActor(db, owner, app.id, 1);

    await applyOperation(db, owner, app.id, { operation: createEntityOp("v2_entity"), baseVersionNumber: 1, idempotencyKey: "k2" });
    await applyOperation(db, owner, app.id, { operation: createEntityOp("v3_entity"), baseVersionNumber: 2, idempotencyKey: "k3" });

    const v1Again = await getVersionForActor(db, owner, app.id, 1);
    expect(v1Again.checksum).toBe(v1.checksum);
    expect(v1Again.payload).toEqual(v1.payload);
  });
});
