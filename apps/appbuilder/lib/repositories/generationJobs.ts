...
export type GenerationOperationBatchRow =
  typeof generationOperationBatches.$inferSelect;
...
export class LeaseLostError extends ConflictError {
  constructor(jobId: string) {
    super(
      `Lease on generation job ${jobId} was lost (reclaimed by another worker or job no longer active)`
    );
...
export class StaleJobStateError extends ConflictError {
  constructor(jobId: string, expected: string) {
    super(
      `Generation job ${jobId} was not in the expected status "${expected}" (concurrent update)`
    );
...
export async function enqueueGenerationJob(
  ...
  return withQuotaRejectionLogging(
    db,
    { category: "generation", appId, actorPrincipalId: actor.principalId },
    () =>
      db.transaction(async (tx) => {
        ...
        if (existing) {
          if (existing.requestHash !== requestHash) {
            throw new ConflictError(
              "Idempotency key reused with a different generation request payload"
            );
          }
          return existing;
        }

        // Pre-M12 per-user cross-app check — kept as-is (not in the M12 quota
        // metric catalogue; a separate, coarser concept than the per-app
        // concurrency quota below).
        ...
        if (
          activeForUser.length >= GENERATION_LIMITS.MAX_ACTIVE_JOBS_PER_USER
        ) {
          throw new ConflictError(
        ...
        // "0 active" and both inserting. Nested with the owner's daily AI-request
        // quota so a single transaction enforces both limits atomically.
        return withQuota(
          tx,
          appId,
          "active_generation_jobs_per_app",
          () => countActiveGenerationJobsForApp(tx, appId),
          () =>
            withQuota(
              tx,
              app.ownerPrincipalId,
              "ai_requests_per_day_per_owner",
              () => countAiRequestsTodayForOwner(tx, app.ownerPrincipalId),
              async () => {
                const [job] = await tx
 ...
 */
export async function claimNextAvailableJob(
  db: Db,
  workerId: string,
  leaseDurationMs: number
): Promise<GenerationJobRow | null> {
  return claimInternal(
    ...
    workerId,
    leaseDurationMs,
    ne(generationJobs.status, "needs_clarification")
  );
}
...
export async function releaseLease(
  ...
  await db
    ...
    .where(
      and(eq(generationJobs.id, jobId), eq(generationJobs.leaseOwner, workerId))
    );
}
...
export async function requestCancellation(
  ...
  return db.transaction(async (tx) => {
    ...
    if (isTerminal(job.status)) {
      throw new ConflictError(
        `Generation job already finished with status "${job.status}"; it cannot be cancelled.`
      );
    }
...
export async function submitClarificationAnswers(
  ...
  return db.transaction(async (tx) => {
    ...
    if (job.status !== "needs_clarification") {
      throw new ConflictError(
        `Generation job is not awaiting clarification (status: ${job.status})`
      );
    }

    const parsedState = ClarificationState.safeParse(
      job.clarificationState ?? { rounds: [] }
    );
    if (!parsedState.success) {
      throw new ConflictError(
        "Generation job's clarification state is corrupted."
      );
    }
    const round = parsedState.data.rounds.find(
      (r: ClarificationRoundType) => r.roundNumber === input.roundNumber
    );
    if (!round) {
      throw new ConflictError(
        `No clarification round ${input.roundNumber} exists on this job.`
      );
    }
    ...
    for (const answer of input.answers) {
      if (!validQuestionIds.has(answer.questionId)) {
        throw new ConflictError(
          `Answer references unknown question id "${answer.questionId}" for this round.`
        );
      }
    }
...
export async function getGenerationJobForActor(
  db: Db,
  actor: Actor,
  appId: string,
  jobId: string
...
export async function getLatestGenerationJobForActor(
  db: Db,
  actor: Actor,
  appId: string
): Promise<GenerationJobRow | null> {
...
export async function listOperationBatchesForActor(
  ...
  return db
    ...
    .where(
      and(
        eq(generationOperationBatches.jobId, jobId),
        eq(generationOperationBatches.appId, appId)
      )
    )
