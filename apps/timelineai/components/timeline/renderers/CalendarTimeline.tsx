...
export function CalendarTimeline({ timeline }: { timeline: RenderableTimeline }) {
  ...
  return (
    <div className="tl-root" data-layout="calendar">
              ...
              <ol role="list" className="grid gap-2 sm:grid-cols-2">
                {groups
                  ...
                  .map((event, index) => (
                    <li
                      key={event.id ?? index}
                      className="rounded-lg border-l-4 bg-[var(--color-surface)] p-3"
      ...
      {undated.length > 0 ? (
        <div className="mt-6">
          <h3 className="mb-2 text-sm font-medium text-[var(--color-text-muted,inherit)]">Undated</h3>
