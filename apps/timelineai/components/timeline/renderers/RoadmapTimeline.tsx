...
export function RoadmapTimeline({ timeline }: { timeline: RenderableTimeline }) {
  ...
  return (
    <div className="tl-root" data-layout="roadmap">
      ...
      <div className="flex flex-col gap-6">
        {[...lanes.entries()].map(([lane, events]) => (
          <section key={lane}>
            <h3 className="mb-2 text-sm font-semibold uppercase tracking-wide text-[var(--color-text-muted,inherit)]">
              {lane}
            </h3>
            <ol role="list" className="flex flex-wrap gap-3">
              {events.map((event, index) => (
                <li
                  key={event.id ?? index}
                  className="min-w-48 flex-1 rounded-lg border-t-4 bg-[var(--color-surface)] p-3"
