...
export const projects: ShowcaseProject[] = [
  {
    ...
    highlights: [
      "M00–M15 shipped as a reviewed PR series: multi-tenant work graph, versioned REST API with OpenAPI, idempotency and optimistic concurrency.",
      "The fast task experience — subtasks, dependencies, estimates, labels, custom fields, recurrence and templates across Inbox, List, Board, Calendar and Timeline — with no AI required.",
      "Human-approved AI proposals convert briefs and notes into tasks, dependencies, owners, and delivery plans.",
      "Next: the creative & useful copilot epic (#244).",
    ],
  },
  {
  ...
  },
  {
    ...
    title: "Testora",
    summary:
      "Live end-to-end test automation: author requirements, suites, fixtures and cases, execute them in a real headless TestCafe browser with streamed progress, and triage failures with screenshots and AI-drafted issues.",
    tags: ["Testing", "Automation", "TestCafe"],
    status: "live",
    glyph: "TS",
    index: "03",
    externalUrl: platformLinks.testora,
    stack: ["Next.js", "TypeScript", "TestCafe", "Drizzle", "PostgreSQL"],
    dependsOn: ["ui", "auth", "own-db", "ai"],
    highlights: [
      "Full authoring model — functional requirements, test suites, fixtures, and cases — persisted in Testora's own Drizzle database, isolated from the shared platform schema.",
      "Signed-in users drive real headless TestCafe runs against per-app local or remote targets, with progress streamed over SSE.",
      "Repeat runs per case and a per-fixture flaky flag that adds retries.",
      "Failure screenshots and AI-drafted issue reports; next up: the autonomous quality loop with TasksAI (#269).",
    ],
  },
  {
    slug: "ai-eval",
